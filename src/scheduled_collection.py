...
def search_and_collect(cycle: str, since_timestamp: str) -> list[dict]:
    ...
    prompt = f"""Search for missile strikes, rocket attacks, drone strikes, airstrikes, ballistic attacks, cruise missile launches, Shahed drone attacks, Houthi attacks, Hezbollah rocket fire, and any weapons launches since {since_timestamp} in 2026 conflicts (Russia-Ukraine, Iran Conflict, US/Israel vs Iran, Houthi/Yemen, Hezbollah/Lebanon, any global). Return a JSON array with these fields per event:
